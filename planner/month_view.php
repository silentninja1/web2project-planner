...
w2PsetMicroTime();
$modclass=W2P_BASE_DIR."/modules/calendar/calendar.class.php";
...
$titleBlock->addCrumb('?m=planner&a=year_view&date=' . $date, 'year view');
$titleBlock->addCrumb('?m=planner&date=' . $date, 'month view');
$titleBlock->addCrumb('?m=planner&a=week_view&date=' . $date, 'week view');
$titleBlock->addCrumb('?m=planner&a=day_view&date=' . $date, 'day view');
...
$titleBlock->addCell('<input type="submit" class="button" value="' . $AppUI->_('new event') . '">', '', '<form action="?m=planner&a=addedit&date=' . $today . '" method="post" accept-charset="utf-8">', '</form>');
$titleBlock->show();
?>

<script language="javascript" type="text/javascript">
function clickDay( uts, fdate ) {
	window.location = './index.php?m=planner&a=day_view&date='+uts+'&tab=0';
}
function clickWeek( uts, fdate ) {
	window.location = './index.php?m=planner&a=week_view&date='+uts;
}
</script>
