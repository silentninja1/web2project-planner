...
?>
            ...
            <table border="0" cellspacing="1" cellpadding="2" width="100%" class="motitle">
                <tr>
                    <td>
                        <a href="<?php echo '?m=planner&date=' . $prev_day->format(FMT_TIMESTAMP_DATE); ?>"><img src="<?php echo w2PfindImage('prev.gif'); ?>" width="16" height="16" alt="pre" border="0"></a>
                    </td>
                   <th width="100%">
                        <?php   echo $AppUI->_(htmlspecialchars($this_day->format('%A'), ENT_COMPAT, $locale_char_set)) . ', ' . $this_day->format($df); ?>
                    </th>                   <td>
                        <a href="<?php echo '?m=planner&date=' . $next_day->format(FMT_TIMESTAMP_DATE); ?>"><img src="<?php echo w2PfindImage('next.gif'); ?>" width="16" height="16" alt="next" border="0"></a>
                    </td>
                </tr>
            </table>
...
*/    
 ...
 /*   */
   ...
   if (perc!=null && perc!='') {
         ...
         $.post('$w2p_base_url/index.php?m=planner&a=test&suppressHeaders=true', { task_id: ".$task['task_id'].", 
           perc_complete: perc, do_task: 'pcpl' }, function(data) {
//        alert('Data Loaded: ' + data);
        if (data==0) {
